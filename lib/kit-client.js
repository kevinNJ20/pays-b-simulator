...
class KitInterconnexionClientMali {
  ...
  async soumettreDeclarationMali(declaration) {
    try {
      ...
      return {
        ...
        prochaine_etape: '17: Kit d\'interconnexion → Sénégal (autorisation mainlevée)'
      };
  ...
  async confirmerArriveeTransit(numeroDeclaration, donneesArrivee) {
    // 🔍 LOGS DE DEBUG COMPLETS
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log('🚛 [KIT-CLIENT] ═══ DÉBUT CONFIRMATION ARRIVÉE TRANSIT ═══');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    console.log(`📋 [KIT-CLIENT] Numéro déclaration: ${numeroDeclaration}`);
    console.log(`📋 [KIT-CLIENT] Données arrivée:`, donneesArrivee);
    console.log(`🌐 [KIT-CLIENT] Base URL: ${this.baseURL}`);
    console.log(`🌐 [KIT-CLIENT] URL complète: ${this.baseURL}/transit/arrivee`);
    console.log(`⏱️  [KIT-CLIENT] Timeout configuré: ${this.timeout}ms`);
    
    try {
      // ✅ Préparation des données
      const arriveeData = {
        numeroDeclaration: numeroDeclaration,
        bureauArrivee: donneesArrivee.bureauArrivee || 'BAMAKO_DOUANES',
        dateArrivee: donneesArrivee.dateArrivee || new Date().toISOString(),
        ...
        visaDouanier: donneesArrivee.visaDouanier || null,
        declarationDetaillee: donneesArrivee.declarationDetaillee || null
      };

      console.log('📤 [KIT-CLIENT] Données à envoyer:', JSON.stringify(arriveeData, null, 2));
      
      const headers = {
        'Content-Type': 'application/json',
        'X-Source-Country': 'MLI',
        'X-Source-System': 'KIT_TRANSIT_MALI_SENEGAL',
        'X-Correlation-ID': `ARRIVAL_MLI_${Date.now()}`,
        'X-Workflow-Step': '14_ARRIVEE_MALI',
        'X-Transit-ID': numeroDeclaration
      };
      
      console.log('📋 [KIT-CLIENT] Headers:', JSON.stringify(headers, null, 2));
      console.log('🚀 [KIT-CLIENT] Envoi requête POST vers MuleSoft...');
      
      const startTime = Date.now();

      // ✅ Appel avec logs détaillés
      const response = await this.client.post('/transit/arrivee', arriveeData, {
        headers: headers,
        timeout: this.timeout,
        validateStatus: function (status) {
          // Accepter tous les status pour les logger
      ...
      console.log(`📥 [KIT-CLIENT] Réponse reçue en ${duration}ms`);
      console.log(`📥 [KIT-CLIENT] Status HTTP: ${response.status} ${response.statusText}`);
      console.log(`📥 [KIT-CLIENT] Headers réponse:`, response.headers);
      console.log(`📥 [KIT-CLIENT] Body réponse:`, JSON.stringify(response.data, null, 2));

      // Vérifier si le status est OK
      if (response.status >= 200 && response.status < 300) {
        console.log('✅ [KIT-CLIENT] ÉTAPES 13-14 TERMINÉES AVEC SUCCÈS');
        console.log('🎯 [KIT-CLIENT] Kit MuleSoft va notifier le Sénégal (ÉTAPES 15-16)');
        console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        return {
          ...
          data: response.data,
          latence: response.metadata?.duration || duration,
          correlationId: response.metadata?.correlationId,
          source: 'MALI_TRANSIT_BAMAKO',
          etapesCompletes: '13-14',
          prochaine_etape: '15-16: Kit interconnexion → Sénégal (confirmation retour)',
          message: 'Confirmation arrivée transmise avec succès vers le Kit d\'interconnexion'
        };
      } else {
        console.error(`❌ [KIT-CLIENT] Erreur HTTP: ${response.status}`);
        throw new Error(`Kit MuleSoft a retourné HTTP ${response.status}: ${response.statusText}`);
      }

    } catch (error) {
      console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.error('❌ [KIT-CLIENT] ═══ ERREUR CONFIRMATION ARRIVÉE ═══');
      console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.error(`❌ [KIT-CLIENT] Type d'erreur: ${error.constructor.name}`);
      console.error(`❌ [KIT-CLIENT] Message: ${error.message}`);
      console.error(`❌ [KIT-CLIENT] Code: ${error.code}`);
      
      if (error.response) {
        console.error(`❌ [KIT-CLIENT] Réponse HTTP: ${error.response.status}`);
        console.error(`❌ [KIT-CLIENT] Status text: ${error.response.statusText}`);
        console.error(`❌ [KIT-CLIENT] Headers:`, error.response.headers);
        console.error(`❌ [KIT-CLIENT] Data:`, error.response.data);
      } else if (error.request) {
        console.error('❌ [KIT-CLIENT] Requête envoyée mais pas de réponse reçue');
        console.error(`❌ [KIT-CLIENT] URL tentée: ${this.baseURL}/transit/arrivee`);
        console.error('❌ [KIT-CLIENT] Détails requête:', {
          method: 'POST',
          url: '/transit/arrivee',
          baseURL: this.baseURL,
          timeout: this.timeout
        ...
        console.error('❌ [KIT-CLIENT] Erreur lors de la préparation de la requête');
        console.error('❌ [KIT-CLIENT] Stack:', error.stack);
      }
      
      console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      
      // Construire un message d'erreur détaillé
      let messageErreur = 'Échec transmission vers kit d\'interconnexion: ';
      
      if (error.code === 'ECONNREFUSED') {
        messageErreur += `Connexion refusée sur ${this.baseURL}/transit/arrivee - Le Kit MuleSoft n'est pas accessible`;
      } else if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
        messageErreur += `Délai d'attente dépassé (${this.timeout}ms) - Le Kit MuleSoft ne répond pas`;
      } else if (error.code === 'ENOTFOUND') {
        messageErreur += `Hôte introuvable: ${this.baseURL} - Vérifiez l'URL du Kit MuleSoft`;
      } else if (error.response?.status === 404) {
        messageErreur += `Endpoint /transit/arrivee non trouvé sur Kit MuleSoft`;
      } else if (error.response?.status >= 500) {
        messageErreur += `Erreur serveur Kit MuleSoft (${error.response.status})`;
      } else if (error.response) {
        messageErreur += `${error.response.status} - ${error.response.statusText}`;
      } else {

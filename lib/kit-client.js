...
class KitInterconnexionClient {
  constructor() {
    // ✅ CORRECTION: URL CloudHub réelle du Kit MuleSoft
  ...
  // === ✅ NOUVELLE MÉTHODE: SOUMISSION DÉCLARATION (DIRECT vers MuleSoft) ===
  async soumettreDeclaration(declaration) {
    try {
      console.log(`📋 [${this.paysCode}] Soumission déclaration vers Kit: ${declaration.numeroDecl}`);
      
      // Préparer les données pour le Kit au format attendu par MuleSoft
      const declarationKit = {
        anneeDecl: declaration.anneeDecl,
        bureauDecl: declaration.bureauDecl,
        numeroDecl: declaration.numeroDecl,
        dateDecl: declaration.dateDecl,
        articles: declaration.articles.map(article => ({
          ...
          nbreColis: article.nbreColis,
          poidsBrut: article.poidsBrut,
          poidsNet: article.poidsNet,
          valeurCaf: article.valeurCaf,
          liquidation: article.liquidation
        }))
      };
      
      // ✅ DIRECT vers MuleSoft - endpoint correct selon RAML
      const response = await this.client.post('/declaration/soumission', declarationKit);
      
      console.log(`✅ [${this.paysCode}] Déclaration soumise avec succès:`, response.data);
      ...
      console.error(`❌ [${this.paysCode}] Échec soumission déclaration:`, error.message);
      
      throw new Error(`Soumission déclaration échouée: ${error.response?.data?.message || error.message}`);
    }
  }
  ...
  async diagnostic() {
    ...
    // ✅ NOUVEAU: Test 4: Test soumission déclaration
    diagnosticResult.tests.soumissionDeclaration = await this.testerSoumissionDeclaration();
    ...
    console.log(`📊 [${this.paysCode}] Diagnostic Kit MuleSoft terminé:`, {
      ...
      declaration: diagnosticResult.tests.soumissionDeclaration?.success
    });
  ...
  // ✅ NOUVEAU: Test de soumission déclaration
  async testerSoumissionDeclaration() {
    try {
      // Simulation d'une déclaration test
      const declarationTest = {
        anneeDecl: "2025",
        bureauDecl: "10S",
        numeroDecl: parseInt(`999${Date.now()}`.slice(-6)),
        dateDecl: new Date().toISOString().split('T')[0],
        articles: [{
          numArt: 1,
          connaissement: `TEST${Date.now()}`.slice(-10),
          modeCond: "TEST",
          codeSh: "9999999999",
          libelleTarif: "Test article pour diagnostic",
          designationCom: "Article de test pour validation du Kit MuleSoft",
          origine: "TEST-PAYS",
          nbreColis: 1,
          poidsBrut: 100,
          poidsNet: 90,
      ...
      await this.soumettreDeclaration(declarationTest);
      
      return {
        success: true,
        message: 'Test soumission déclaration réussi'
      };
    } catch (error) {
      return {
        success: false,
